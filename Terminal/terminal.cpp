...
static TCHAR DevFileName[]       = TEXT( "emc_2_0_monitor_only.flash" );
static TCHAR DevHexoutFileName[] = TEXT( "emc_chip_02000000.hexout.flash" );
...
SOCKET_MONITOR BoardMonitor;

bool       Handshaking = false;
bool       HandshakeFileUploadInProgress = false;
FILE_CLASS HandshakeFile;
int        PlusesNeeded = 0;
...
*                          HANDSHAKE_CALLBACK                          *
***********************************************************************/
void handshake_callback( char c )
{
static char turn_off_pluses[] = "P0\r";
static STRING_CLASS s;
static int retrys = 0;
TCHAR * cp;
TCHAR * pp;

if ( c == QuestionChar && !s.isempty() )
    {
    retrys++;
    if ( retrys > 5 )
        {
        MessageBox( 0, "Download Aborted", "5 Retrys", MB_OK );
        HandshakeFileUploadInProgress = false;
        HandshakeFile.close();
        BoardMonitor.send( turn_off_pluses );
        retrys = 0;
        return;
        }
    }
    ...
    /*
    --------------------------------
    Download aborted by socket error
    -------------------------------- */
    HandshakeFileUploadInProgress = false;
    HandshakeFile.close();
    return;
...
if ( PlusesNeeded < 1 )
    ...
    while ( true )
        ...
        if ( !cp )
            ...
            PlusesNeeded = 0;
            BoardMonitor.send( turn_off_pluses );
            set_text( MainEbox, EmptyString );
            MainListBox.set_current_index( MainListBox.add( TEXT("Send complete.")) );
            return;
            }

        BytesSent += (lstrlen(cp) + 2 )*sizeof(TCHAR);
        ...
        if ( *cp != NullChar )
            ...
            if ( s.len() > 0 )
                {
                set_text( MainEbox, int32toasc((int32) BytesSent) );
                s += CrString;
                BoardMonitor.send( s.text() );
                break;
                }
            }
        }
...
/***********************************************************************
*                        UPLOAD_FILE_W_HANDSHAKE                       *
***********************************************************************/
void upload_file_w_handshake( STRING_CLASS filename )
{
static char turn_on_pluses[] = "P1\r";
STRING_CLASS s;

TotalBytesInFile = file_size( filename.text() );
BytesSent        = 0;

if ( HandshakeFile.open_for_read(filename.text()) )
    {
    s = TEXT( "Sending " );
    s += filename;
    s += TEXT( " with handshake..." );
    MainListBox.add( s.text() );
    s = TEXT( "Total Bytes = " );
    s += TotalBytesInFile;
    MainListBox.set_current_index( MainListBox.add( s.text()) );
    PlusesNeeded = 2;
    HandshakeFileUploadInProgress = true;
    BoardMonitor.send( turn_on_pluses );
    }
else
    {
    s = TEXT("File name =[" );
...
switch (msg)
    ...
    case WM_COMMAND:
        ...
        if ( cmd == BN_CLICKED )
            ...
            else if ( id == DEV_FILE_PB )
                {
                set_text( hWnd, UPLOAD_FILE_NAME_EBOX, DevFileName );
                }
            else if ( id == DEV_HEXOUT_FILE_PB )
                {
                set_text( hWnd, UPLOAD_FILE_NAME_EBOX, DevHexoutFileName );
                }
            }
        break;
    }
...
*                          SHOW_SOCKET_ERROR                           *
***********************************************************************/
static void show_socket_error()
{
STRING_CLASS s;
s = TEXT( "Socket Error = ");
s += BoardMonitor.last_error();
MainListBox.add( s.text() );
}
...
if ( nd )
    ...
    switch ( nd->type )
        {
        case SOCKET_ERROR_TYPE:
            if ( HandshakeFileUploadInProgress )
                handshake_callback( AChar );
            show_socket_error();
            break;
        ...
        case ASCII_TYPE:
            ...
            if ( HandshakeFileUploadInProgress )
                {
                if ( nd->len == 1 && (*cp==PlusChar || *cp==QuestionChar || *cp==HatChar) )
                    {
                    if ( *cp != HatChar )
                        handshake_callback( *cp );
                    add_to_listbox = false;
                    }
                }
...
switch ( msg )
    ...
    case WM_COMMAND:
        switch (id)
            {
            case CONNECT_CHOICE:
                cp = already_connected;
                if (!BoardMonitor.is_running() )
                    {
                    BoardMonitor.begin_monitoring( MainWindow );
                    if ( BoardMonitor.open_socket( TcpIpAddress, TcpIpPort) )
                        cp = connected;
                    else
                        cp = fail_to_connect;
                    }
            ...
            case STOP_CHOICE:
                BoardMonitor.close_socket( close_callback );
                return 0;

            case EXIT_CHOICE:
                BoardMonitor.stop_monitoring( MainWindow );
                return 0;
            ...
            case UPLOAD_FILE_W_HANDSHAKING_CHOICE:
                 if ( id == UPLOAD_FILE_W_HANDSHAKING_CHOICE )
                    Handshaking = true;
                 else
                    Handshaking = false;
    ...
    case WM_CLOSE:
        ...
        if ( BoardMonitor.is_monitoring() )
            {
            MainListBox.set_current_index( MainListBox.add( TEXT("Closing Socket...")) );
            BoardMonitor.stop_monitoring( MainWindow );
            return 0;
            }
        break;
