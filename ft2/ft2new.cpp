...
if ( GetFocus() == SetDacDialog.control(LVDT_DIGITAL_POT_EBOX) )
    {
    send_lvdt_digital_pot_command();
    return;
    }
...
if ( file_exists(s.text()) )
    ...
    if ( s != UNKNOWN )
        s.set_text( w, LS1_DESCRIPTION_TBOX );

    s = ini.get_string( LS2DescriptionKey );
    if ( s != UNKNOWN )
        s.set_text( w, LS2_DESCRIPTION_TBOX );

    s = ini.get_string( LS3DescriptionKey );
    if ( s != UNKNOWN )
        s.set_text( w, LS3_DESCRIPTION_TBOX );

    s = ini.get_string( LS4DescriptionKey );
    if ( s != UNKNOWN )
        s.set_text( w, LS4_DESCRIPTION_TBOX );

    s = ini.get_string( LS5DescriptionKey );
    if ( s != UNKNOWN )
        s.set_text( w, LS5_DESCRIPTION_TBOX );

    s = ini.get_string( LS6DescriptionKey );
    if ( s != UNKNOWN )
        s.set_text( w, LS6_DESCRIPTION_TBOX );
    }
}
...
if ( file_exists(s.text()) )
    ...
    set_text( w, LS1_DESCRIPTION_TBOX, SureTrakParameters.limit_switch_desc(0) );
    set_text( w, LS2_DESCRIPTION_TBOX, SureTrakParameters.limit_switch_desc(1) );
    set_text( w, LS3_DESCRIPTION_TBOX, SureTrakParameters.limit_switch_desc(2) );
    set_text( w, LS4_DESCRIPTION_TBOX, SureTrakParameters.limit_switch_desc(3) );
