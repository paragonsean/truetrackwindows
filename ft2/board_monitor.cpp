...
if ( do_not_upload && UploadRetractTimeout && (do_not_upload_timeout==0) )
    {
...
if ( f.open_for_read(nc.text()) )
    ...
    /*
    ----------------------------------------------
    Tell TrueTrak V7 software to reload everything
    ---------------------------------------------- */
    if ( version > 5 )
        {
        send_socket_message( TEXT("V447=0"), false );
        send_socket_message( TEXT("V448=0"), false );
        }
        ...
        if ( Boards[i].waiting_for_ack || Boards[i].waiting_for_plus )
            ...
            if ( !I_Am_Shutting_Down && Boards[i].is_monitoring() )
                ...
                if ( ms < Boards[i].ms_at_last_contact )
                    ...
                    /*
                    -------------------------------------------------------------------
                    If the time counter has wrapped just reset the time of last contact
                    and wait for it to time out again.
                    ------------------------------------------------------------------- */
...
if ( start_chars_are_equal(CLEAR_DO_NOT_UPLOAD_CMD, command) )
    {
    if ( b->do_not_upload )
        {
        b->do_not_upload = false;
        b->do_not_upload_timeout = false;
        set_text( Dnu[i], EmptyString );
        }
    return;
    }
