...
***********************************************************************/
void load_realtime_machine_list( void )
{
COMPUTER_CLASS c;
DB_TABLE    t;
...
if ( c.count() > 1 )
    ...
    t.open( machset_dbname(c.whoami()), MACHSET_RECLEN, PFL );
...
*                            LOAD_MACHLIST                             *
***********************************************************************/
static void load_machlist( void )
{
COMPUTER_CLASS c;
...
*                            LOAD_PART_INFO                            *
***********************************************************************/
static BOOLEAN load_part_info( void )
{
...
*                       SELECT_PRINTER_FONT                            *
***********************************************************************/
static void select_printer_font( void )
{
TEXTMETRIC tm;
...
*                         DELETE_PRINTER_FONT                          *
***********************************************************************/
static void delete_printer_font( void )
{
if ( PrinterFont )
    {
...
*                          RESET_PLOT_CURVES                           *
***********************************************************************/
static void reset_plot_curves( void )
{
int i;
...
*                          SELECT_MYFONT                               *
***********************************************************************/
static void select_myfont( void )
{
TEXTMETRIC tm;
...
***********************************************************************/
static void erase_left_cursor_line( void )
{
...
*                             ROD_HEAD_LABEL                           *
***********************************************************************/
static void rod_head_label( void )
{
...
*                             DRAW_X_AXIS                              *
***********************************************************************/
void draw_x_axis( MINMAX &limits, int decimal_places,
...
***********************************************************************/
void adjust_all_y_axis_limits( void )
{
int  channel;
int  i;
int  x;
...
***********************************************************************/
static void show_corner_parameters( void )
{
if ( MyScreenType == BOX_SCREEN_TYPE || MyScreenType == TREND_SCREEN_TYPE || MyScreenType == BIG_PARAM_SCREEN_TYPE )
    return;
...
for ( i=0; i<CurrentMarks.n; i++ )
    ...
    switch ( me->parameter_type )
        ...
        case START_POS_MARK:
             show_pos_mark( CurrentPart.user_avg_velocity_start[me->parameter_number-1], me );
            break;
...
*                           PAINT_THE_CURVES                           *
***********************************************************************/
static void paint_the_curves( void )
{
...
***********************************************************************/
void paint_big_param_display( PAINTSTRUCT & ps )
{
...
*                                                                      *
***********************************************************************/
void size_rectangles_to_current_display( void )
{
...
*                                                                      *
***********************************************************************/
void size_profile_window( RECT mr )
{
...
***********************************************************************/
void size_plot_window_to_screen( void )
{
RECT  mr;
...
***********************************************************************/
static int printer_zoom_box_height( void )
{
...
*   This assumes you have already created the printer font.            *
***********************************************************************/
static int printer_parameter_box_height( void )
{
const int  lineheight = 1;
TEXTMETRIC tm;
int        ch;
...
*                            PRINT_ZOOM_BOX                            *
***********************************************************************/
static void print_zoom_box( void )
{
TEXTMETRIC tm;
int        col;
int        row;
...
***********************************************************************/
static void show_printer_parameters( void )
{
const int  lineheight = 1;
...
*                           TURN_OFF_ZOOMING                           *
***********************************************************************/
static void turn_off_zooming( void )
{
...
***********************************************************************/
static void set_zoom_grayness( void )
{
UINT  item[] = {
    ZOOM_MENU,
    UNZOOM_MENU,
...
if ( IsHPM )
    ...
    if ( MyScreenType == STANDARD_SCREEN_TYPE || MyScreenType == ZABISH_SCREEN_TYPE )
        ...
        if ( CurrentDisplay == POS_TIME_DISPLAY || CurrentDisplay == POS_ONLY_DISPLAY )
            {
            load_menu( MainWindow, TEXT("profile"), CurrentPasswordLevel, bad_pos_ids, NOF_BAD_POS_IDS );
            have_menu = true;
            }
        else if ( CurrentDisplay == TIME_ONLY_DISPLAY )
            {
            load_menu( MainWindow, TEXT("profile"), CurrentPasswordLevel, bad_time_ids, NOF_BAD_TIME_IDS );
            have_menu = true;
            }
        }
    }
...
if ( !have_menu )
    load_menu( MainWindow, TEXT("profile"), CurrentPasswordLevel, bad_other_ids, NOF_BAD_OTHER_IDS );
...
***********************************************************************/
void toggle_pos_time_display_type( void )
{
...
static TCHAR SmallCsDialog[] = TEXT( "SMALL_CHOOSE_SHOTS" );
...
if ( ChosshotWindow )
    ...
    ChosshotWindow = CreateDialog( MainInstance, cp, MainWindow, (DLGPROC) chosshot_proc );
    }
}
...
*                             SET_ZOOM_AXIS                            *
***********************************************************************/
void set_zoom_axis( int ax, int left, int right, int region_index )
{
RECT r;
float arange;
float xrange;
...
*                                UNZOOM                                *
***********************************************************************/
void unzoom( BOOLEAN need_update )
{
clear_axes();
...
***********************************************************************/
void position_main_plot_window( void )
{
RECT pbr;
RECT r;
HWND w;
...
***********************************************************************/
static void fill_corner_param_listbox( void )
{
short i;
short j;
BOOLEAN is_corner;
...
{
TCHAR s[MAX_STRING_LEN+1];   /* MAX_STRING_LEN >= PARAMETER_NAME_LEN */
...
if ( box == otherbox )
    ...
    /*
    if ( get_lb_count(CornerParamDialogWindow, otherbox) >= NofCornerParameters )
        {
        LoadString( MainInstance, LISTBOX_FULL_MESSAGE, s, sizeof(s) );
        LoadString( MainInstance, CANT_DO_THAT_STRING, t, sizeof(t) );
        MessageBox( CornerParamDialogWindow, s, t, MB_OK );
        return;
        }
    */
    }
...
status = LB_ERR;
x = SendDlgItemMessage(CornerParamDialogWindow, box, LB_GETCURSEL, 0, 0L);
if ( x != LB_ERR )
    status = SendDlgItemMessage( CornerParamDialogWindow, box, LB_GETTEXT, x, (LPARAM)(LPSTR) s );

if ( status != LB_ERR )
    {
    SendDlgItemMessage( CornerParamDialogWindow, box, LB_DELETESTRING, x, 0L );
    SendDlgItemMessage( CornerParamDialogWindow, otherbox, LB_ADDSTRING, 0, (LPARAM)(LPSTR) s  );
    }
...
***********************************************************************/
void position_corner_param_dialog()
{
WINDOW_CLASS    w;
RECTANGLE_CLASS dr;
RECTANGLE_CLASS r;
...
cp = get_ini_string( TEXT("visitrak.ini"), TEXT("ButtonLevels"), TEXT("EditOperatorsButton") );
...
***********************************************************************/
static void do_keyboard_zoom_button( void )
{
POINT p;
...
***********************************************************************/
void toggle_current_display_type( void )
{
int new_display_type;
...
*                                                                      *
***********************************************************************/
static void show_last_screen_type( void )
{
TCHAR * cp;
short   i;

cp = get_ini_string( TEXT("display.ini"), TEXT("Config"), TEXT("LastScreenType") );
...
***********************************************************************/
void do_view_parameters_menu( void )
{
turn_off_zooming();
...
if ( t.open( graphlst_dbname(ComputerName, MachineName, PartName), GRAPHLST_RECLEN, PFL) )
    {
...
***********************************************************************/
static void register_for_current_machine( void )
{
...
***********************************************************************/
static void unregister_for_current_machine( void )
{
...
***********************************************************************/
static void unregister_for_new_shot_events( void )
{
...
if ( MachList.find(NewSetupMachine) )
    ...
    --------------------- */
    cp = machine_computer_name( NewSetupMachine );
    if ( cp )
        {
        MachList.add( cp, NewSetupMachine, SORT_BY_MACHINE );
        if ( MachList.find(NewSetupMachine) )
            register_for_current_machine();
        ...
        --------------------------------------------------------------------- */
        if ( strings_are_equal(cp, c.whoami()) )
            {
...
***********************************************************************/
static BOOLEAN append_overlay_shot( void )
{
int i;
...
*                         REDISPLAY_LAST_SHOT                          *
***********************************************************************/
BOOLEAN redisplay_last_shot( void )
{
TCHAR * cp;
...
***********************************************************************/
static BOOLEAN get_alarm_date_extents( void )
{
...
*                          PRINT_ALARM_SUMMARY                         *
***********************************************************************/
static void print_alarm_summary( void )
{
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case IDOK:
                ...
                STRING_CLASS dest;
                STRING_CLASS sorc;
                sorc = profile_ii_name( ComputerName, MachineName, PartName, ShotName );
                dest.get_text( hWnd, CSV_SHOT_FILE_EBOX );
                if ( is_checked(hWnd, SAVE_RAW_XBOX) )
...
char    cpath[MAX_PATH+1+MACHINE_NAME_LEN+1+PART_NAME_LEN+1];
TCHAR * cp;
...
t.open( plookup_dbname(ComputerName,MachineName), PLOOKUP_RECLEN, PFL );
...
***********************************************************************/
void create_menu_help_box( void )
{
...
*                                                                      *
***********************************************************************/
static int main_menu_width( void )
{
HWND desktop_window;
RECT r;
...
*                          SAVE_SCREEN_TYPES                           *
***********************************************************************/
static void save_screen_types( void )
{
TCHAR * cp;
...
***********************************************************************/
static void check_for_monitor_program( void )
{
TCHAR path[MAX_PATH+1];

if ( get_exe_directory(path) )
    {
    lstrcat( path, MonallProgramName );
    HaveMonitorProgram = file_exists( path );
    }
}
