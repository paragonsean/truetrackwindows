...
FTII_VAR_ARRAY CurrentChannelModeVar;
HPEN           MasterPen;
...
/*
----------------------------
Get the default channel mode
---------------------------- */
s.get_exe_dir_file_name( ComputerName.text(), ft2_control_program_name(version, CurrentMachine) );
CurrentChannelModeVar.get_from( s.text() );
...
***********************************************************************/
static void set_channel_5_8_labels( bool need_refresh )
{
...
if ( mymode != CHANNEL_MODE_7 )
    {
    for ( i=0; i<3; i++ )
        {
        w = CurrentValuesDialog.control( CH_3_TBOX + i );
        w.set_title( int32toasc(3+i) );
        }
...
if ( checked )
    ...
    for ( i=0; i<3; i++ )
        {
        w = CurrentValuesDialog.control( CH_6_TBOX + i );
        w.set_title( int32toasc(6+i) );
        }
...
for ( i=0; i<6; i++ )
    ...
    if ( checked )
        {
        if ( (i==0 && mymode==CHANNEL_MODE_7) || (i==3 && mymode==CHANNEL_MODE_5) )
            {
...
for ( id=MIN_VOLTS_TBOX; id<=MAX_VOLTS_TBOX; id++ )
    {
    w = CurrentValuesDialog.control(id);
    if ( mymode==CHANNEL_MODE_7 || mymode == CHANNEL_MODE_5 )
        w.show();
    else
        w.hide();
    }
...
if ( CurrentWindow == SetDacDialog && IsRevisionD )
    {
...
/***********************************************************************
*                      HAS_NEW_EXTERNAL_INTERFACE                      *
***********************************************************************/
static BOOLEAN has_new_external_interface()
{
int version;
version = current_version();
if ( version > 4 )
...
switch ( wParam )
    ...
    case OP_STATUS_TYPE:
        ...
        if ( CurrentOpStatusIsEmpty || CurrentOpStatus != *opstat )
            ...
            if ( has_new_external_interface() )
                {
                /*
                ------------------------------------------
                The upper values are zero if the mode is 7
                ------------------------------------------ */
                if ( channel_mode == CHANNEL_MODE_7 )
                    {
                    CurrentOpStatus.analog[8]  = CurrentOpStatus.analog[4];
                    CurrentOpStatus.analog[10] = CurrentOpStatus.analog[5];
                    CurrentOpStatus.analog[12] = CurrentOpStatus.analog[6];
                    CurrentOpStatus.analog[13] = CurrentOpStatus.analog[7];
                    }

                /*
                ----------------------------
                channels 1-8, 17-24, 0,0,0,0
                ---------------------------- */
                for ( i=0; i<8; i++ )
                    copy_analog( i, CurrentOpStatus.analog[i] );

                for ( i=8; i<16; i++ )
                    copy_analog( i+8, CurrentOpStatus.analog[i] );
                }
            else
                {
                for ( i=0; i<8; i++ )
                    {
                    x = (unsigned) CurrentOpStatus.analog[i];
                    channel = x & 0xF000;
                    channel >>= 12;
                    if ( channel>11 && channel_mode>CHANNEL_MODE_3 )
                        channel += 4;
                    x &= 0xFFF;
                    CurrentAnalogValue[channel] = x;
                    }
                }
...
CurrentChannelModeVar.upsize( 1 );
CurrentChannelModeVar[0].set( CHANNEL_MODE_VN, 0 );
...
/*
----------------------------------------------------
Channel 17 (index 16) is wired to the lvdt feedback.
---------------------------------------------------- */
i = 16;
...
is_rev_d = has_new_external_interface();
is_digital_servo_amp = FALSE;
if ( CurrentMachine.monitor_flags & MA_HAS_DIGITAL_SERVO )
    is_digital_servo_amp = TRUE;
...
-------------------------------------------------------------------------------
                                Measured Command
Channel 18 (index 17) is wired to the command volt input to the servo amp board
------------------------------------------------------------------------------- */
if ( is_rev_d )
    {
    if ( mymode == CHANNEL_MODE_7 )
...
/*
-------------------------------------------------
TB2 #23 (index 16) is wired to the LVDT feedback.
for external servo amp boards else use mode 7
------------------------------------------------- */
if ( mymode == CHANNEL_MODE_7 )
    {
    i = 4;
...
else if ( mymode == CHANNEL_MODE_5 )
    {
    i = 6;
    }
else if ( is_rev_d )
    {
    if ( is_digital_servo_amp )
        i = 12;
    else
        i = 13;
    }
...
if ( is_rev_d )
    ...
    /*
    ---------------------------------------------------------
    Channel 18 (index 17) is wired to the raw lvdt feedback )
    --------------------------------------------------------- */
    i = 17;
    if ( mydac == 2 )
        i = 19;
...
***********************************************************************/
static void send_lvdt_digital_pot_command()
{
...
switch (msg)
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case AMP_1_RADIO:
            case AMP_2_RADIO:
                if ( !UpdatingSetDacScreen && IsRevisionD )
                    {
                    if ( id == AMP_1_RADIO )
                        {
...
if ( file_exists(s.text()) )
    ...
    limit_switch_number = ini.get_int( VacuumLimitSwitchKey );
    }
else
    {
...
ParkStepNum = TEXT( ".022 " );
ParkStepVolts = TEXT( "-1000" );
...
/***********************************************************************
*                        GET_PARK_STEP_FROM_LINE                       *
*               .022 UD1=-2000   #Set valve to something               *
***********************************************************************/
static void get_park_step_from_line( TCHAR * sorc )
{
TCHAR * cp;
...
/***********************************************************************
*                  LOAD_CONTROL_MODS_FROM_UPLOAD_FILE                  *
***********************************************************************/
static bool load_control_mods_from_upload_file()
{
NAME_CLASS   s;
FILE_CLASS   f;
TCHAR      * cp;
...
if ( f.open_for_read(s.text()) )
    ...
    while ( true )
        ...
        if ( have_park_step && have_alt_step )
            break;
        }
    f.close();
    }
...
Park Valve
---------- */
set_text( w, VOLTAGE_EBOX, ft2_volts_toascii(ParkStepVolts.text()) );
show_park_step();
...
{
if ( !load_control_mods_from_file() )
    load_control_mods_from_upload_file();
copy_mods_to_screen();
}
...
switch (msg)
    ...
    case WM_COMMAND:
        switch ( id )
            {
            case CURRENT_DIGITAL_POT_EBOX:
                if ( cmd == EN_CHANGE && !UpdatingValveTestScreen )
                    send_current_digital_pot_command();
                return TRUE;
...
for ( i=0; i<8; i++ )
    ...
    if ( AnalogValue[chan] != CurrentAnalogValue[chan] || NeedToRefreshAnalogs || NeedRefresh )
        ...
        if ( chan < 2 || (channel_mode != CHANNEL_MODE_7 && chan < 16) )
            {
...
***********************************************************************/
static void send_channel_mode_command()
{
STRING_CLASS s;
int checked_mode;
int unchecked_mode;
...
if ( IsRevisionD )
    checked_mode = CHANNEL_MODE_7;
else
    checked_mode = CHANNEL_MODE_5;
...
if ( is_checked(SwitchChannelsXbox) )
    ...
    {
    unchecked_mode = CurrentChannelModeVar[0].ival();
    if ( unchecked_mode != checked_mode )
        s += unchecked_mode;
    else
        s += CHANNEL_MODE_1;
    }
